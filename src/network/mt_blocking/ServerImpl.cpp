...
void ServerImpl::_process_connection(int client_socket) {
    ...
    while (running.load()) {
        try {
            ...
            std::string error_msg("SERVER ERROR ");
            error_msg += ex.what();
            error_msg += "\r\n";
            if (send(client_socket, error_msg.data(), error_msg.size(), 0) <= 0) {
...
void ServerImpl::OnRun() {
    while (running.load()) {
        ...
        if ((client_socket = accept(_server_socket, (struct sockaddr *)&client_addr, &client_addr_len)) == -1) {
            ...
            _logger->debug("Dont accept {}\n", client_socket);
            _logger->warn("Dont accept {}\n", client_socket);
        ...
        if (_cur_workers.load() < _n_workers) {
            ...
            _logger->warn("Close connection {} {} {} \n", client_socket, _cur_workers.load(), _n_workers);
